...
/*   Updated: 2021/02/04 15:45:31 by trouchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_init_sprites(t_map *map)
{
	...
	if (!(map->spr.pos = malloc(sizeof(t_pos) * (map->spr.nbr + 1))))
		return (0);
	if (!(map->spr.zbuffer = malloc(sizeof(double) * (map->resolution[0]))))
		return (0);
...
void	ft_init_sprite_engine2(t_map *map)
{
	...
	if (map->spr.drawendx >= map->resolution[0])
		map->spr.drawendx = map->resolution[0] - 1;
}
...
void	ft_init_sprite_engine(t_map *map, int i)
{
	...
	map->spr.invdet = 1.0 / (map->ray.planex * map->ray.diry
	...
	* (1 + map->spr.transformx / map->spr.transformy));
	...
	ft_init_sprite_engine2(map);
}

void	ft_sprites2(t_map *map, int *k, int *z)
{
	while ((*k) < (map->spr.drawendx))
	{
		...
		&& map->spr.zbuffer[*k - 1] > map->spr.transformy)
		{
